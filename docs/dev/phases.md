...
**ドッグフーディング基準:** 2026 年分の実データで、年間取引報告書・税務 SaaS の損益計算結果と突合して差分ゼロ（または差分を説明可能）

### 計画・意思決定

- [x] ロードマップ策定（`docs/dev/tax-roadmap.md`）
- [x] ADR-004: 損益計算ロジックを CLI 内 `tax` カテゴリに例外として実装
- [ ] API 履歴保持期間ほか前提リスク 6 項目の実機確認（Week 1 最優先）
- [ ] 税制調査結果の反映（総平均/移動平均・手数料・信用・円換算・端数処理）
- [ ] 正規化フォーマットの決定（Cryptact / Gtax 直接出力 vs 汎用）

### 実装

- [ ] 生データ取得の網羅化（全ペア横断・入出金ページング・JST 年境界ヘルパー）
- [ ] 正規化コマンド（段階 2）＋ CLI Skill（A 層向け）
- [ ] 損益計算コマンド（段階 3・tax カテゴリ）＋ Skill 統合（B 層向け）
- [ ] CLAUDE.md / commands.md への tax カテゴリ・例外条項追記
- [ ] MCP Tool 化（CLI サブプロセス方式、C 層向け対話フロー・免責・read-only キー導線）
