...
<!-- TODO: セクション構成確定後、下記 MCP リンクを MCP「はじめに」の正式 URL に差し替える。現状は MCP ドキュメントサイトのルートを指している。 -->
より詳しい比較は、MCP 側の「はじめに」を参照してください → [bitbank MCP ドキュメント](https://bitbank-lab.gitbook.io/bitbank-lab-docs/)
