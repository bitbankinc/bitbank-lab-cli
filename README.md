...
- **MCP サーバー** ([bitbank-lab-mcp](https://github.com/bitbankinc/bitbank-lab-mcp)) はサーバー側で計算済みの結論を LLM に渡す
...
以降の README 内コマンド例は `bitbank` が PATH に通っている前提です。skills の改造やコントリビュートにはクローン手順（`./install.sh`）を使います（[開発](#開発) 参照）。

## Plugin としてインストールする

Skill（自然言語操作の指示書）は、必要に応じて plugin で追加します。plugin に含まれるのは **Skills だけ**で、実処理を担う CLI 本体は含まれません。**どの導線でも [クイックスタート](#クイックスタート) の `npm i -g bitbank-lab-cli` が別途必須**です（plugin cache 同梱の `bin/bitbank` は依存を含まず直接実行できません）。

Plugin 登録が必要なのは「**リポジトリを開いていない場所でも Skill を使いたい**」場合だけです。Claude Code / Cursor でこのリポジトリを clone して開くなら、Skill は自動で有効になるため登録不要です（[想定する使い方](#想定する使い方) 参照）。

> 注: `/plugin install` はローカル版 Claude Code CLI（ターミナル）用の slash command です。Web 版（[claude.ai/code](https://claude.ai/code)）はコンテナが一時的なため動作しません。
...
Plugins（Customize）画面の「Search or Paste Link」に GitHub URL を貼って Import します。`skills/` 配下の Skill 一式が user scope（全プロジェクト共通）で登録されます。

```text
https://github.com/bitbankinc/bitbank-lab-cli
```

- plugin の実体は Claude Code と同じ `~/.claude/plugins/` 配下で共有されます。Claude Code 側で `/plugin install` 済みなら、追加操作なしで有効になっていることがあります（一覧に `Imported` と表示）
- plugin は git commit 固定でキャッシュされ、自動更新されません。新しい Skill を取り込むには Plugins 画面から update または再 import してください
...
- `<plugin>@<marketplace>` は決め打ちせず、**`codex plugin list` の表示をそのまま使ってください**（`codex plugin install <owner>/<repo>` というサブコマンドはありません）
- 実行中に `~/.codex` への書き込み承認を求められたら **Yes** で進めます。macOS は Xcode Command Line Tools が必要です（[前提条件](#前提条件) 参照）
...
旧 Gemini CLI 環境からは `agy plugin import gemini` で移行できます（本リポジトリは新旧両方の manifest を同梱）。自動 install したくない場合は [Codex CLI / Antigravity CLI で使う](#codex-cli--antigravity-cli-で使う) の手動コピー手順も使えます。
...
- **Codex CLI / Antigravity CLI など** — 各エージェント固有のパスに Skill をコピーすれば自動トリガー可能（[後述](#codex-cli--antigravity-cli-で使う)）。配置しない場合も `AGENTS.md` 経由で CLI を呼ばせられる
...
[クイックスタート](#クイックスタート) を参照（CLI を使うだけなら `npm i -g bitbank-lab-cli`、skills を改造する場合は [開発](#開発) のクローン手順）。
...
### Codex CLI / Antigravity CLI で使う

Codex CLI / Antigravity CLI など、`.claude/skills/` を読まないエージェントで Skill を自動トリガーさせたい場合は、各エージェントが見るパスにコピーまたはシンボリックリンクを張ります。

| エージェント | 配置先 |
|---|---|
| Codex CLI | `.agents/skills/<name>/SKILL.md` |
| Antigravity CLI | `.agents/skills/<name>/SKILL.md`（旧 Gemini CLI の `.gemini/skills/` は読まれない） |
...
あくまでサンプルですので、ご自身の用途に合わせて追加・編集してください。Codex CLI / Antigravity CLI など他エージェントでの配置先は [使い方](#codex-cli--antigravity-cli-で使う) を参照してください。
