...
- **macOS**: `codex plugin marketplace add` は内部で `git clone` を実行するため、Xcode Command Line Tools が未導入だと `xcode-select: No developer tools were found` で失敗します。先に以下を実行してください:

  ```bash
  xcode-select --install
  ```

- CLI 本体（`npm i -g bitbank-lab-cli`）は別途必要です（[前提条件](#前提条件) / 上記レイヤー表を参照）。
