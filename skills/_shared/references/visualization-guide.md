...
- **保存した画像を提示前に一度確認し、タイトル・ラベル・注記の重なりや
  見切れがあればレイアウトを調整して描き直す**。とくにドーナツ・円系で
  小さいスライスの外周ラベルが密集する場合は、凡例方式への切替や
  タイトルの pad 追加で回避する。描き直しで不要になった下書きファイルは、
  同一ターン内で自分が作ったものに限り削除してよい（それ以外の整理は
  ユーザーに任せる）
