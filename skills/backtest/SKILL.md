...
- バックテストの約定は次の足の始値での成行を想定するため、デフォルトは
  **taker レート**を片道分として使う。ユーザーが指値（maker）前提を明示した
  場合のみ maker レートを使う（bitbank の maker は負＝リベートになり得る）
- レートは campaign 等で随時更新されるため、実行のたびに取得する
- 取得に失敗した場合のみ taker 0.12%（0.0012）にフォールバックし、
  結果出力に「手数料はフォールバック値」と明記する
...
ペア: BTC/JPY
時間軸: 1day
手数料: taker 0.12%（片道、bitbank pairs 現行値）
...
- **手数料を必ず考慮する。** 手数料を無視するとバックテスト結果が大幅に楽観的になり、実運用で損失につながる。レートは `bitbank pairs` の現行値を使い、ハードコードした過去の数字（例: 旧 maker 0.12%。現行 maker はリベートで負）を使い回さない。取得失敗時のみ taker 0.12% にフォールバックし、その旨を結果に明記する
