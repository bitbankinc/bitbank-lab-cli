...
// (cli/commands/schema・confirm-guard・cli/error-codes・cli/exit-codes・
// skills/*/SKILL.md 可視化節) から生成する。
// committed と regenerate の差分ゼロを検査し、コマンド追加・エラーコード変更・
// チャート表変更時の取り込み漏れ（"7 vs 12" 級のズレ）を CI で止める。
...
describe("Chaos X-17: agents/ catalogs are generated, not hand-edited (anti-drift)", () => {
  ...
  it("agents/chart-catalog.json matches the generator output", () => {
    expect(
      serialize(buildChartCatalog()),
      "agents/chart-catalog.json is stale. Run `npx tsx scripts/gen-agents-catalog.ts` and commit.",
    ).toBe(committed("chart-catalog.json"));
  });
});
